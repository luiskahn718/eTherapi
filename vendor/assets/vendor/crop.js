	...
	return function () {
		...
		this.imgSize = function () {
			...
			if (imgSize.w * holderRatio.hw < imgSize.h * holderRatio.wh) {
				...
				this.imgInfo.w = c.width() - (size*2);
				this.imgInfo.h = this.imgInfo.w * (imgSize.h / imgSize.w);
				this.imgInfo.al = size;

			} else {

				this.imgInfo.h = c.height() - (size*2);
				this.imgInfo.w = this.imgInfo.h * (imgSize.w / imgSize.h);
				this.imgInfo.at = size;
			}
		...
		this.imgMove = function (move, callback) {
			var img = this.eles.img,
				imgInfo = this.imgInfo,
				c = this.eles.container,
				size = 40;
			...
			if (t > size) {
				t = size;
				imgInfo.t = (imgInfo.at == size) ? 0 : -size;
			} else if (t < -((imgInfo.h * imgInfo.s) - (c.height() - size))) {
				t = -((imgInfo.h * imgInfo.s) - (c.height() - size));
				imgInfo.t = ((imgInfo.at == size) ? (imgInfo.h * imgInfo.s) - (c.height() - size*2) : (imgInfo.h * imgInfo.s) - (c.height() - size));
			}

			if (l > size) {
				l = size;
				imgInfo.l = (imgInfo.al == size) ? 0 : -size;
			} else if (l < -((imgInfo.w * imgInfo.s) - (c.width() - size))) {
				l = -((imgInfo.w * imgInfo.s) - (c.width() - size));
				imgInfo.l = ((imgInfo.al == size) ? (imgInfo.w * imgInfo.s) - (c.width() - size*2) : (imgInfo.w * imgInfo.s) - (c.width() - size));
			}
		...
		*/
		...
		coordinates = function coordinates(self) {
			...
			var imgInfo = self.imgInfo,
				...
				coordinates = {
					x: -(parseInt(img.css('left')) - size) * (imgInfo.aw / (imgInfo.w * imgInfo.s)),
					y: -(parseInt(img.css('top')) - size) * (imgInfo.ah / (imgInfo.h * imgInfo.s)),
					w: (c.width() - (size*2)) * (imgInfo.aw / (imgInfo.w * imgInfo.s)),
					h: (c.height() - (size*2)) * (imgInfo.ah / (imgInfo.h * imgInfo.s)),
					image: imgsrc,
				};;
